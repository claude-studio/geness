...
| R-005 | target-root and project identity follow-up depends on unresolved OQ-006/OQ-013 details. | `medium` | C-01 fork/rekey policy is selected, but schema migration, config precedence와 symlink policy의 final schema가 미확인이다. | Storage/Schema ADR에서 ADR-0015 identity boundary, projection과 config boundary를 함께 검증한다. | user | `open` |
